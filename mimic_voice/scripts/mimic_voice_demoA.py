...
WHAT MIMIC_VOICE_DEMOA PROGRAM DOES

Mimic_voice_demoA uses pyttsx to make Baxter talk
1) Baxter says hello and welcomes everyone
2) Baxter demos his grippers
3) Baxter blinks the LED lights on his arm and his halo
4) The halo lights go from green to red
5) Then Baxter starts the mimic part of the program
...
if __name__ == '__main__':
    try:
        rospy.init_node('mimic_voice_basic', anonymous=True)
        mime = MimicVoiceA()
        ...
        #Instantiate an object of the class Gripper so that we can 
        # demo Baxter's grippers
        grip = mimic_voice.Gripper()

        # Call the start_test function which uses threading so that
        # Baxter's grippers simultaneously calibrate that open and close. 
        grip.demo_gripper()
        rospy.sleep(1)

        mime.move_to_position(pi)

        #Instantiate an object of the HaloBlink class to demo Baxter's lights
        pi.talk_lights()
        lights = mimic_voice.HaloBlink()
        lights.demo_halo_blink()
        lights.talk_mimic()
