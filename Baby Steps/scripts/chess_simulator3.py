...
class Locate():
    def __init__(self, arm, distance):
        ...
        self.ball_tolerance = 0.1 # 0.005
        self.tray_tolerance = 0.2 #.05
...
def load_gazebo_model_ball(ball_pose = Pose(position = Point(x = 0.6, y = 0.25, z = 0.815)), ball_reference_frame = "world"):
    # Get Models' Path
    print ("in the load ball model function")
    model_path = rospkg.RosPack().get_path('baby_steps')+"/models/"
    # Load ball SDF
    ball_xml = ''
    with open (model_path + "ball/model.sdf", "r") as ball_file:
...
def load_gazebo_model_ball1(ball_pose = Pose(position = Point(x = 0.6, y = 0.22, z = 0.815)), ball_reference_frame = "world"):
    # Get Models' Path
    print ("in the load ball model function")
    model_path = rospkg.RosPack().get_path('baby_steps')+"/models/"
    # Load ball SDF
    ...
    try:
        spawn_sdf = rospy.ServiceProxy('/gazebo/spawn_sdf_model', SpawnModel)
        resp_sdf = spawn_sdf("ball1", ball_xml, "/",
                             ball_pose, ball_reference_frame)
    except rospy.ServiceException, e:
        rospy.logerr("Spawn SDF service call failed: {0}".format(e))
...
def delete_gazebo_models():
    ...
    try:
        ...
        resp_delete = delete_model("ball1")
    except rospy.ServiceException, e:
...
if __name__ == "__main__":
    ...
    print('\n' * 3)
    board_corners = list() 
    print ("Going to load ball model")
    load_gazebo_model_ball()
    print("loaded ball model")

    print ("Going to load ball1 model")
    load_gazebo_model_ball1()
    print("loaded ball1 model")
    rospy.sleep(20)
